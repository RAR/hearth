...
import com.rar.hearth.sendspin.sendspin.audio.AudioSink
import com.rar.hearth.sendspin.sendspin.audio.AudioTrackSink
...
class SyncAudioPlayer(
    ...
    private val latencyEstimator = com.rar.hearth.sendspin.sendspin.latency.OutputLatencyEstimator(
        nowNs = nowNs,
    )
    ...
    fun initialize() {
        ...
        try {
            ...
            latencyEstimator.start { result ->
                when (result) {
                    is com.rar.hearth.sendspin.sendspin.latency.OutputLatencyEstimator.Result.Converged -> {
                        timeFilter.setAutoMeasuredDelayMicros(
                            result.latencyMicros,
                            com.rar.hearth.sendspin.sendspin.latency.StaticDelaySource.AUTO,
                        )
                        AppLog.Audio.d("[delay-cal] converged: ${result.latencyMicros}us from ${result.sampleCount} samples")
                    }
                    is com.rar.hearth.sendspin.sendspin.latency.OutputLatencyEstimator.Result.TimedOut -> {
                        timeFilter.setAutoMeasuredDelayMicros(
                            0L,
                            com.rar.hearth.sendspin.sendspin.latency.StaticDelaySource.NONE,
                        )
    ...
    private fun handleStartGatingDacAware(track: AudioSink): Boolean {
        ...
        if (latencyEstimator.status == com.rar.hearth.sendspin.sendspin.latency.OutputLatencyEstimator.Status.Measuring) {
